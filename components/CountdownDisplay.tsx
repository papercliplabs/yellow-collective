...
export const DurationDisplay = ({ duration }: { duration: number }) => {
    const renderSegmentText = (segmentName: keyof typeof splitDurationSegments, segmentValue: number) => {
        if (segmentValue === 0) {
            return "";
        }
        if (segmentValue === 1) {
...
export const CountdownDisplay: React.FC<CountdownDisplayProps> = (props: CountdownDisplayProps) => {
    const [timeLeft, setTimeLeft] = useState<{
        ...
        getTimeLeft(getNumber(props.to), getNumber(props.from === undefined ? new Date().getTime() / 1000 : props.from))
    );
