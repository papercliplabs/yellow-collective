        ...
        <Layout>
            ...
            <div className="items-center w-full grid grid-cols-3 gap-4 mt-12">
                <div className="w-full bg-skin-muted border border-skin-stroke rounded-xl p-6">
                    <ProgressBar label="For" type="success" value={forVotes} percentage={getVotePercentage(forVotes)} />
                </div>
                <div className="w-full bg-skin-muted border border-skin-stroke rounded-xl p-6">
                    <ProgressBar
                        label="Against"
            ...
            <div className="mt-12">
                <div className="text-2xl font-heading text-skin-base font-bold">Description</div>
...
const ProposedTransactions = ({ target, value, calldata }: { target: string; value: number; calldata: string }) => {
...
const VoteButton = ({ proposal, proposalNumber }: { proposal: Proposal; proposalNumber: number }) => {
    ...
    return (
        <Fragment>
            <ModalWrapper className="w-full max-w-lg bg-skin-muted" open={modalOpen} setOpen={setModalOpen}>
                <VoteModal proposal={proposal} proposalNumber={proposalNumber} setOpen={setModalOpen} />
            </ModalWrapper>
            <button
    ...
    return (
        <div className="w-full">
            <div className="flex flex-col items-center sm:items-start sm:flex-row justify-between mb-1">
                ...
                <div className="font-semibold text-xl mt-4 sm:mt-0 text-center sm:text-left">{value}</div>
            </div>
            <div className={`w-full ${bgColor} rounded-full h-4 mt-4 sm:mt-0`}>
                <div className={`${baseColor} h-4 rounded-full`} style={{ width: `${percentage}%` }}></div>
            </div>
        </div>
    );
};
